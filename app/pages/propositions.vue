  ...
  <div class="flex flex-col gap-4">
    ...
    <div class="tile">
      ...
      >
        ...
        <span v-if="item.before && item.after && item.count > 1" class="mono text-[11.5px]">
          {{ item.count }} séances
        </span>
        <span class="mono text-[11.5px] text-text-dim">
          {{ formatDate(item.createdAt.slice(0, 10)) }}
        </span>
        <span v-if="item.before && item.after" class="mono text-[12.5px] text-text-dim">
        ...
        <span v-else class="mono text-[12.5px] text-text-dim">{{ item.count }} ajustements</span>
