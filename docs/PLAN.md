...
**Où on en est (16 sept. 2026)** : P0, P0.5, P1 et P2 livrés (dernier commit `38abb52` « P2 — réalisé, retour de séance, charge » ; Strava abandonné, voir la décision dans le bloc P2). Nuxt 4 + Tailwind 4 + Pinia + nuxt-auth-utils + Drizzle/Neon, coque desktop complète, moteur `fitness` / `running` / `plan` / `load` / `matching` testé, seed des données réelles du § 0, écrans Courses, Profil, Semaine, Course à pied, Connexions et Cockpit avec réalisé, retour de séance et charge. Une revue d'entraîneur du plan généré (16 sept.) a relevé des défauts du moteur course, corrigés dans la brique **P1.5** (livrée). P3, P3.5, P3.6, P4, P5 et P5.6 livrés, les trois parcours de P5 vérifiés dans le navigateur. La semaine porte ses quatre courses, ses trois musculations et sa sortie vélo ; l'Imprévu, la déclaration de pause et la recherche de course tournent sur `claude-sonnet-5` en effort `low` (§ 6). Les briques P5.7 et P5.8 sont écrites et en attente. Suivent **P5.6** (placement vélo et lendemain de sortie longue), puis **P5.7** (identité : âge, profil physique, avatar), **P5.8** (tuiles pleine largeur et tuiles ouvrantes) et **P5.9** (séances de muscu réelles et temps de récupération), décidées avec Ronan le 16 sept. 2026, puis **P5.10** (édition d'une course) et **P5.11** (projection complète et confiance), écrites le 17 sept. 2026 après revue de ce qui manquait aux courses. Les deux dernières s'appuient sur les dialogs de P5.8 : ne pas les attaquer avant.
...
- [ ] `domain/races` : `racePlansChanged(before, after)` — comparaison pure des seuls champs qui pilotent la périodisation (date, distance, priorité, mode d'objectif). Un objectif chiffré, un D+, une température attendue ou une note n'en font pas partie : ils changent l'affichage, pas le plan. Tests : les quatre champs déclenchent, les quatre autres non.
- [ ] `PlanTrigger.RaceEdited` (`course_modifiee`) : valeur ajoutée à l'enum Postgres `plan_trigger`, migration générée. Une modification ne régénère le plan que si `racePlansChanged` est vrai.
- [ ] `server/api/races/[id].put.ts` : même schéma zod que la création (nom, date, distance, priorité, mode d'objectif, objectif, D+, température attendue, notes), `objectifS` forcé à nul en performance maximale. `source`, `status`, `resultatS`, `representative` et le `race_lookup` rattaché ne sont pas touchés ; une course dont le statut n'est pas « planifiée » refuse la modification. Tests : objectif fixé sans régénération, date reculée avec régénération, course courue refusée.
- [ ] Dialog **Course** (P5.8) : passe en lecture et écriture, chaque champ modifiable en place, l'ancienne valeur barrée dès qu'elle change (§ 8), une seule action principale « Enregistrer ». Quand la modification régénère le plan, le dialog le dit avant d'enregistrer. La suppression quitte la ligne de la table pour devenir l'action secondaire du dialog, derrière une confirmation.
- [ ] Table Courses : la ligne ouvre le dialog ; « à fixer » devient un appel à l'action qui ouvre directement le champ objectif.
- [ ] Vérifier dans le navigateur au scénario `bloc-2` : fixer l'objectif de Paris met à jour l'écart dans la table et le cadran Course A sans régénérer le plan ; reculer Madrid d'une semaine régénère le plan et décale le mini-cycle ; le `race_lookup` de Madrid est toujours rattaché après modification.
- [ ] Fini : lint, typecheck, tests, build verts ; § 8 relu ; commit « P5.10 — édition d'une course ».
