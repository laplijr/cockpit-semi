...
export default defineEventHandler(async (event) => {
  ...
  const [habits, weeks] = await Promise.all([
    db
      .select()
      .from(habit)
      .where(eq(habit.athleteId, athleteId))
      .orderBy(desc(habit.confidence), desc(habit.detectedAt)),
    db
      .select()
      .from(calibration)
      .where(eq(calibration.athleteId, athleteId))
      .orderBy(desc(calibration.date))
      .limit(8),
  ])
